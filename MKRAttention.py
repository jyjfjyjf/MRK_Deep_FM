...
class MKRAttention(nn.Module):
    def __init__(self, dim, user_num, item_num, relation_num, so_num, low_layer_num, high_layer_num):
        ...
        user_config = BertConfig(vocab_size=user_num,
                                 hidden_size=8,
                                 num_hidden_layers=1,
        ...
        item_config = BertConfig(vocab_size=item_num,
                                 hidden_size=8,
                                 num_hidden_layers=1,
                                 num_attention_heads=2,
                                 intermediate_size=48,
        ...
        self.user_transformer = BertModel(user_config)
        self.item_transformer = BertModel(item_config)

        # self.user_output = MLP(16, dim)
        # self.item_output = MLP(16, dim)
    ...
    def forward(self,
                ...
                flag=True,
                labels=None):
        item_embedding = self.item_embedding(item_ids.view(-1, 1))
        item_embedding = self.item_transformer(inputs_embeds=item_embedding).last_hidden_state.squeeze(1)
        # item_embedding = self.item_output(item_embedding)
        head_embedding = self.so_embedding(s_ids)
        item_embeddings, head_embeddings = self.cc_unit(item_embedding.squeeze(1), head_embedding)

        if flag:
            user_embedding = self.user_embedding(user_ids.view(-1, 1))
            user_embedding = self.user_transformer(inputs_embeds=user_embedding).last_hidden_state.squeeze(1)
            # user_embedding = self.user_output(user_embedding)
            # user_embedding = self.user_mlp(inputs_embeds=user_embedding).last_hidden_state.squeeze(1)
